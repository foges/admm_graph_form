...
template <typename T>
void matrix_add_constant_diag(matrix<T> *A, T val) {
  uint numel = std::min(A->size1, A->size2);
  uint block_dim = std::min(kBlockSize, numel);
  uint grid_dim = calc_grid_dim(numel, kBlockSize);
  __matrix_add_constant_diag<<<grid_dim, block_dim>>>(A->data, val, A->tda);
}
